...
for (const type of formTypes) {
	events.on(`${type}:errors`, (data: FormErrors) => {
		...
		const str = Object.keys(data)
			.map((key: keyof IFormData) => data[key])
			.join('; ');
...
for (const type of formTypes) {
	events.on(
		...
		(data: { field: keyof IFormData; value: string }) => {
			...
			if (appState.order.setFormField(data).hardValidate(type)) {
	...
	events.on(`${type}:submit`, () => {
		...
		if (type === 'order') {
			...
			api
				.orderProducts(appState.order.getFormData())
				.then((result) => {
					wipeData();
					modal.render({
						content: success.render({ description: result.total }),
					});
				})
				.catch((error) => {
					console.error('Error during order processing');
					console.error(error);
					events.emit('contacts:errors', {
						email: `Ошибка при обработке заказа: ${error}`,
					});
				});
		}
		// }
